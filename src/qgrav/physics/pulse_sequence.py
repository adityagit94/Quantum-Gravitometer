...
class MachZehnderPulseSequence:
    ...
    def pulse_durations_s(self) -> tuple[float, float, float]:
        ...
        return (float(self.tau_pi_half_s), 2.0 * float(self.tau_pi_half_s), float(self.tau_pi_half_s))
...
def vibration_amplitude_axis(amplitude_min_m: float, amplitude_max_m: float, n_points: int) -> np.ndarray:
    if n_points < 5:
        raise ValueError('Vibration amplitude sweep requires at least 5 points.')
    return np.linspace(float(amplitude_min_m), float(amplitude_max_m), int(n_points), dtype=np.float64)
