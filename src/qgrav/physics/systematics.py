...
def gravity_gradient_shift_m_s2(
    gradient_per_m: float | None = None,
    ...
    if gradient_per_m is None:
        gradient_per_m = FREE_AIR_GRADIENT.value
    effective_drop = abs(drop_height_m) + 0.5 * NOMINAL_GRAVITY.value * interferometer_time_s**2
...
def coriolis_shift_m_s2(
    ...
    return 2.0 * OMEGA_EARTH.value * abs(horizontal_velocity_m_s) * abs(float(np.cos(lat_rad)))
