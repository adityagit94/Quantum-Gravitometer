...
def _check_gravity_sweep(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not gravity_propagation:
        ...
        checks.append(build_check('analytic_phase_match', bool(max_abs <= 1e-10),
                                  observed=max_abs, expected='<= 1e-10 rad'))
        checks.append(build_check('phase_linearity', bool(r2 >= 0.999999),
                                  observed=r2, expected='>= 0.999999'))
    else:
        ...
        visibility = float(
            (np.max(p3) - np.min(p3)) / max(np.max(p3) + np.min(p3), 1e-15)
        )
...
def _check_multi_drop_cycle(result: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = [
        ...
        build_check(
            ...
            bool(len(taus) == len(adev) and len(taus) >= 1
                 and np.all(taus > 0) and np.all(adev >= 0)),
