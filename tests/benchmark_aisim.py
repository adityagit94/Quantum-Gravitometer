...
def _ensemble(n_atoms=1000):
    _, atoms, _, _, _ = _create_detected_ensemble(
        n_atoms=n_atoms, seed=1, cloud_radius_m=3e-3,
...
def test_benchmark_single_mz(benchmark):
    ...
    benchmark(
        lambda: _run_mach_zehnder_sequence(
            atoms, tau_pi_half_s=23e-6, interferometer_time_s=0.26,
            intensity_profile=beam, wave_vectors=wv, final_phase_rad=1.0,
        )
    )
...
def test_benchmark_gravity_sweep_hybrid(benchmark):
    benchmark(
        lambda: run_aisim_gravity_sweep(
            n_atoms=1000, seed=1, n_gravity_points=61,
            gravity_span_m_s2=6e-6, gravity_propagation=False,
        )
    )
...
def test_benchmark_gravity_sweep_emergent(benchmark):
    benchmark(
        lambda: run_aisim_gravity_sweep(
            n_atoms=1000, seed=1, n_gravity_points=61,
            gravity_span_m_s2=6e-6, gravity_propagation=True,
        )
    )
...
def test_benchmark_multi_drop_emergent(benchmark):
    benchmark(
        lambda: run_aisim_multi_drop_cycle(
            n_drops=100, n_atoms=1000, seed=1,
            interferometer_time_s=0.26, gravity_propagation=True,
        )
    )
