...
def _gs_cfg(**overrides):
    cfg = dict(
        enabled=True, backend="aisim", model="gravity_sweep",
        n_atoms=150, seed=3, n_gravity_points=7, gravity_span_m_s2=4e-6,
        interferometer_time_s=0.05, tau_pi_half_s=23e-6,
        lock_to_midfringe=True, gravity_propagation=False,
    )
...
class TestMachZehnderPhaseScanYAML:
    def test_mz_phase_scan_single_photon_via_yaml(self):
        cfg = dict(
            enabled=True, backend="aisim", model="mach_zehnder_phase_scan",
            n_atoms=150, seed=3, n_phase_points=11,
            interferometer_time_s=0.05, tau_pi_half_s=23e-6,
        )
