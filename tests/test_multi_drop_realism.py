...
class TestPIDServo:
    ...
    def test_zero_error_holds(self):
        st = servo_pid_step(PIDServoState(), population=0.5, setpoint=0.5, kp=0.5, ki=0.1, kd=0.0)
    ...
    def test_proportional_sign(self):
        ...
        st = servo_pid_step(PIDServoState(), population=0.7, setpoint=0.5, kp=1.0, ki=0.0, kd=0.0)
    ...
    def test_anti_windup_clamps_integral(self):
        ...
        for _ in range(1000):
            st = servo_pid_step(
                st, population=1.0, setpoint=0.5, kp=0.0, ki=1.0, kd=0.0, integral_clamp=10.0
            )
    ...
    def test_derivative_responds_to_change(self):
        ...
        st2 = servo_pid_step(st, population=0.6, setpoint=0.5, kp=0.0, ki=0.0, kd=1.0)
...
class TestPIDServoInMultiDrop:
    ...
    def test_pid_servo_reduces_offset_vs_open_loop(self):
        ...
        assert off_pid < off_open, (
            f"PID did not improve lock: open-loop |dP3|={off_open:.3f}, " f"PID |dP3|={off_pid:.3f}"
        )
    ...
    def test_pid_servo_locks_near_midfringe(self):
        ...
        assert abs(steady_mean - 0.5) < 0.08, f"PID steady-state mean P3 = {steady_mean}"
...
class TestCorrelatedVibration:
    ...
    def test_correlated_vibration_is_deterministic(self):
        ...
        np.testing.assert_array_equal(r1["vibration_phase_rad"], r2["vibration_phase_rad"])
    ...
    def test_correlated_vibration_more_low_freq_than_white(self):
        ...
        assert r_corr > r_white, f"correlated tail ratio {r_corr:.3f} not > white {r_white:.3f}"
